...
public class UIAtlasMaker : EditorWindow
{
	...
	List<Texture> GetSelectedTextures ()
	{
		...
		if (Selection.objects != null && Selection.objects.Length > 0)
		{
			...
			foreach (Object o in objects)
			{
				...
				else if(NGUISettings.atlas.texture == null)
				{
					textures.Add(tex);
				}
				else
				{
					// 由于已经指向了rgb的贴图，就是拆分过的，所以要排除alpha的依赖
					string name = NGUISettings.atlas.texture.name;
					name = name.Replace("_rgb", "");
					string rgbname = name + "_rgb";
					string alphaname = name + "_alpha";
					if(NGUISettings.atlas.texture == tex ||
						tex.name == rgbname ||
						tex.name == alphaname)
					{
						continue;
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (PackTextures(tex, sprites))
		{
			...
			string path = System.IO.Path.GetDirectoryName(newPath);
	...
	void OnGUI ()
	{
		...
		{
			//GUILayout.BeginHorizontal();
			NGUISettings.atlasPMA = false; //EditorGUILayout.Toggle("PMA Shader", NGUISettings.atlasPMA, GUILayout.Width(100f));
			//GUILayout.Label("Pre-multiplied alpha", GUILayout.MinWidth(70f));
			//GUILayout.EndHorizontal();
		}

		//GUILayout.BeginHorizontal();
		NGUISettings.unityPacking = true; //EditorGUILayout.Toggle("Unity Packer", NGUISettings.unityPacking, GUILayout.Width(100f));
		//GUILayout.Label("or custom packer", GUILayout.MinWidth(70f));
		//GUILayout.EndHorizontal();

		//GUILayout.BeginHorizontal();
		NGUISettings.trueColorAtlas = true; //EditorGUILayout.Toggle("Truecolor", NGUISettings.trueColorAtlas, GUILayout.Width(100f));
...
//		GUILayout.BeginHorizontal();
		NGUISettings.allow4096 = false;//EditorGUILayout.Toggle("4096x4096", NGUISettings.allow4096, GUILayout.Width(100f));
...
//		GUILayout.EndHorizontal();
		...
		else
		{
			...
			if (create)
			{
				...
				{
//					if(!path.StartsWith(NGUIAtlasSplitAlpha.atlasGeneratePath))
//					{
//						EditorUtility.DisplayDialog("错误", "不能保存在这个目录，请保存在"+ NGUIAtlasSplitAlpha.atlasGeneratePath + "或者它的子目录", "我知道了");
//						return;
//					}
//					else
					{
...
//				// 这里处理一下ui贴图的分离逻辑，如果是删除，修改，添加sprite
