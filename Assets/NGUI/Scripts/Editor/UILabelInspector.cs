...
public class UILabelInspector : UIWidgetInspector
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		EditorGUI.BeginDisabledGroup(!isValid);
		{
			...
			if (mFontType == FontType.Unity) {
				// Use Emojis?
				GUILayout.BeginHorizontal();
				sp = NGUIEditorTools.DrawProperty("UseEmojis", serializedObject, "mUseEmojis", GUILayout.Width(100f));
                if (sp.boolValue)
                {
					if (NGUIEditorTools.DrawPrefixButton("Atlas"))
						ComponentSelector.Show<UIAtlas>(OnSelectAtlas);
					SerializedProperty atlas = NGUIEditorTools.DrawProperty("", serializedObject, "mEmojiAtlas", GUILayout.MinWidth(20f));
	...
	{
		serializedObject.Update();
		SerializedProperty sp = serializedObject.FindProperty("mEmojiAtlas");
		sp.objectReferenceValue = obj;
		serializedObject.ApplyModifiedProperties();
