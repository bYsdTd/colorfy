...
public class UILabel : UIWidget
{
    ...
    [HideInInspector] [SerializeField] UIAtlas mEmojiAtlas = null;
	
    [HideInInspector][SerializeField] EmojiSprite mEmojiSprite;
	public EmojiSprite emojiSprite {
		get {
			if (!useEmojis)
				return null;
			...
			if (mEmojiSprite == null) {
				...
				if (emojiTrans == null) {
					...
					emojiTrans.position = Vector3.zero;	
				} else {
					mEmojiSprite = emojiTrans.GetComponent <EmojiSprite>();
				}
				
				if (mEmojiSprite == null) {
					mEmojiSprite = emojiTrans.gameObject.AddComponent<EmojiSprite>();
                    mEmojiSprite.label = this;
                    mEmojiSprite.pivot = Pivot.TopLeft;
                    mEmojiSprite.SetAnchor(gameObject, 0, 0, 0, 0);
                    mEmojiSprite.depth = depth + 1;
                }
			}
	...
	public override void MarkAsChanged ()
	{
		...
		// Refresh the emoji when something changes
		if (useEmojis) {
			mEmojiSprite.MarkAsChanged();
		}
	}
	...
	protected virtual void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
				...
				if (mOverflow == Overflow.EllipsisContent && !fits && mText.Length != mProcessedText.Length)
				{
					...
					for (int i = 0; i < len; ++i)
					{
						// bug 7416, [名称]名称里包含Emoji表情，在世界地图上查看该城市就会崩溃
						//倒着往前，如果碰到两字节的emoji，要跳过两个char
						char c = truncated[len-1-i];
						int removeCount = 1;

						if(c>=0xD800 && c<=0xDFFF && i < (len-1))
						{
							removeCount = 2;
							++i;
						}

						removeCount = firstTry ? removeCount : removeCount + ellipsis.Length;
						...
						{
							mCalculatedSize = NGUIText.CalculatePrintedSize(candidate);

                            // 删掉只剩半截的表情代号
                            int cutIndex = candidate.Length - ellipsis.Length;
                            int index = truncated.LastIndexOf("[", cutIndex, cutIndex);
							if (index < 0 || !useEmojis)
                                return;
                            var emoji = emojiSprite.MatchSymbol(truncated, index, cutIndex);
							if (emoji != null && index + emoji.length > cutIndex) { // 表情被截断
                                mProcessedText = sb.Remove(index, cutIndex - index).ToString();
                            }
                            return;
						}
					}
		...
		if (full)
		{
			...
			if (useEmojis)
			{
				emojiSprite.ResetAnchors();
				emojiSprite.UpdateAnchors();
			}
#endif
		}
	}
	...
	/// Prints the emojis.
	/// </summary>
	public void PrintEmojis (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols) {
		if (!isValid) return;

		string text = processedText;
		UpdateNGUIText();

		NGUIText.PrintEmojis(text, verts, uvs, cols);
	...
	public string GetNumFormat(NumFormatType formatType)
	{
		string str = string.Empty;
		switch (formatType) {
		case NumFormatType.FormatK:
			{
				str = "K";
			}
			break;
		case NumFormatType.FormatM:
			{
				str = "M";
			}
			break;
		case NumFormatType.FormatB:
			{
				str = "B";
			}
			break;
	...
	public string FormateNumberText(string input, NumShowType showType)
	{
		long number = 0;
		...
		else if(showType == NumShowType.Short)
		{
			if(Math.Abs(number) >= 1000000000)
			{
				...
				return string.Format("{0:F1}" + GetNumFormat(NumFormatType.FormatB), newNum);
			}
			else if(Math.Abs(number) >= 1000000)
			{
				long temp = number / 100000;
				float newNum = temp / 10f;
				return string.Format("{0:F1}" + GetNumFormat(NumFormatType.FormatM), newNum);
			}
			else if(Math.Abs(number) >= 1000)
			{
				long temp = number / 100;
				float newNum = temp / 10f;
				return string.Format("{0:F1}" + GetNumFormat(NumFormatType.FormatK), newNum);
			}
			else
			{
				return input;
	...
	public void UpdateNGUIText ()
	{
		...
		NGUIText.fontScale = isDynamic ? mScale : ((float)mFontSize / mFont.defaultSize) * mScale;
		NGUIText.useEmojis = useEmojis;
		if (useEmojis)
			NGUIText.emojiSprite = emojiSprite;
