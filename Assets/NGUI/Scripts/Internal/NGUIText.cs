...
static public class NGUIText
{
	...
	static public bool useEmojis = false;
	static public EmojiSprite emojiSprite = null;
	...
	static public void Update (bool request)
	{
        ...
        useSymbols = useEmojis || (bitmapFont != null && bitmapFont.hasSymbols) && encoding && symbolStyle != SymbolStyle.None;
	...
	{
		if (useEmojis) {
			return GetEmoji(text, index, textLength);
		} else if (useSymbols) {
	...
	static public BMSymbol GetEmoji (string text, int index, int textLength)
	{
		return (emojiSprite != null) ? emojiSprite.MatchSymbol(text, index, textLength) : null;
	}
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				else
				{
					...
					if (useEmojis && symbol.height > currentLineHeight)
					{
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount)
	{
		...
		for (; offset < textLength; ++offset)
		{
			...
			if (Mathf.RoundToInt(remainingWidth) < 0)
			{
				...
				if (lineIsEmpty || lineCount == maxLineCount || eastern)
				{
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
                ...
                // Align to bottom
                if (useEmojis && finalLineHeight != currentLineHeight)
                {
                    CenterAlign(verts, currentLineStartIndex, verts.size, (currentLineHeight - finalLineHeight) / 2);
                }
	...
	static void CenterAlign (BetterList<Vector3> verts, int start, int end, float offset) {
